...
**State lives in one Pinia store.** `stores/app.ts` (`useAppStore`) is the heart of the app — it holds the current user, all activity/post/conversation/credit lists, and nearly all business logic as actions (`handleJoinActivity`, `handleProcessJoinRequest`, `handleInitiateChat`, `publishActivity`, `publishPost`, etc.). Pages are mostly thin views over this store.

**Auth and posts are real; the rest is still mock.** The **authentication module** (login / register / SMS login) calls the real backend (`docs/API.md`) through the store actions `loginWithPassword` / `loginWithSms` / `registerUser` / `logout` / `restoreSession`, persists the Sa-Token + IM credentials via `uni.setStorageSync` (keys in `config`), logs into Tencent Cloud IM with the returned `userSig`, and auto-restores the session on launch (`pages/index/index.vue` → `restoreSession`). The **posts module** (forum / detail / publish / my posts / favorites) is also backend-backed (`docs/API.md §4`) via `api/post.ts` — pages consume the backend VOs directly (`PostListItemVO` / `PostDetailVO` / `CommentVO`) with page-local pagination; the store only caches categories (`postCategories` / `loadCategories`) and wraps publishing (`publishPost` → `createPost` + credit log). **Everything else** (activities / conversations / credit logs) is still **in-memory mock**, seeded from `data/mock.ts` on launch and reset on reload — no persistence, no backend for that data yet.

**Mock-first for the remaining business data.** `data/mock.ts` provides seed content and `data/types.ts` defines every domain type (`UserProfile`, `Activity`, `Conversation`, `GroupJoinRequest`, the auth VOs `UserLoginVO`/`UserInfoVO`/`RegisterParam`, and the post VOs `PostListItemVO`/`PostDetailVO`/`CommentVO`/`ForumCategoryVO`/`CreatePostParam`/`PageResult<T>`, etc.). A new *mock business* feature (activities/chat) generally means: add/extend a type, seed mock data, add a store action — not a network call. A new *backend-backed* feature means: add an `api/*` function and either map the VO into an in-store shape (auth, see `mapVoToProfile`) or consume the VO directly in the page (posts). Backend list/detail reads live in pages with local pagination state; only cross-cutting concerns (credit score, category cache) go through the store.

**Navigation is two-layered.** There is the native uni-app TabBar — **5 tabs: 首页 (home) / 论坛 (forum) / 发布 (publish-center) / 消息 (chat) / 我的 (profile)**, declared in `pages.json` — switched with `uni.switchTab`. The middle **发布** tab is `pages/publish-center/publish-center.vue`, a chooser that navigates on to `pages/publish-activity/publish-activity.vue` (发起活动 → `publishActivity` action) or `pages/publish-post/publish-post.vue` (发布帖子 → `publishPost` action). *Separately*, individual pages keep their own in-page sub-tab state (e.g. `home.vue` has 推荐 / AI偏好 / 附近 / 主题周 driven by a local `currentTab`). `store.activeTab` exists but the canonical page switching is via `uni.*` navigation APIs, not the store. App entry flows through `pages/index/index.vue`, which redirects to `pages/verify/verify.vue` (the **login page** — phone+password / phone+SMS only; SMS login on an unregistered phone auto-enters a **single-page 基本信息 register form** — 昵称/真实姓名/密码/性别 required, 兴趣标签/头像 optional, **no 学籍认证**; the avatar uploads via an Aliyun OSS presigned URL through `utils/upload.ts`) when there is no `userProfile`, otherwise to home. `utils/upload.ts` now wraps the **common presign endpoint** `POST /oss/presign` (`docs/API.md §3.9`, whitelist dirs `avatar`/`post`/`chat`): `uploadAvatar` (dir `avatar`) and `uploadPostMedia`/`uploadPostImage` (dir `post`, used by the publish-post page) share one `getPresignedUploadUrl(dir, ext)` → PUT-to-OSS path; remote `http(s)` URLs pass through unchanged.
...
- `config/index.ts` currently has **real `dev` values** filled in (`API_BASE_URL` `http://47.105.57.88:8080`, `IM_SDK_APP_ID`, `CAPTCHA_ID`); the **`prod` block is still placeholder** (`IM_SDK_APP_ID: 0`, empty `CAPTCHA_ID`) — fill it before a production/发行 build. Note `IM_SDK_APP_ID: 0` triggers the silent IM-skip path in `utils/im.ts`.
