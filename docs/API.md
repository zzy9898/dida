...
```
POST /users/1001/verifyIdcard
```

```json
{
  "name": "张三",
  "idCard": "433589200609082356"
}
```
...
| 结果 | 响应                                           |
| --- |------------------------------------------------|
| 认证成功 | `code: 200`，`message: "验证通过"`             |
| 验证失败（姓名与身份证号不匹配） | `code: 404`，`message: "验证失败"`             |
| 系统/接口异常（第三方核验接口返回非 `10000` 或调用异常） | `code: 500`，`message: "系统异常，请稍后重试"` |

### 3.7 学籍认证

学信网在线验证码核验。服务端用 `verifyCode` 请求学信网在线验证页面，解析出「姓名 / 学校名称 / 学籍状态」，并要求**解析出的姓名与该用户注册时填写的真实姓名一致**、且学籍状态包含「在籍」才算通过。
...
| --- |------------------------------------------------|
| 认证成功 | `code: 200`，`message: "验证通过"`             |
| 验证失败（学信网姓名与本人不符，或学籍状态非「在籍」） | `code: 404`，`message: "验证失败"`             |
| 该校不在服务范围内 | `code: 404`，`message: "该校不在服务范围内"`   |
| 系统/接口异常（请求学信网失败） | `code: 500`，`message: "系统异常，请稍后重试"` |
